...
export function demoProvider() {
  ...
  return {metrics,async evaluate(state,questions){
    ...
    return Object.fromEntries(Object.entries(questions).map(([id,q])=>[id,['sufficient','purpose_sufficient'].includes(id)?1:q.instructions.purpose_id?purposes[state.segment_id]?.[q.instructions.purpose_id]??.03:tables[state.segment_id]?.[q.instructions.category.split(' > ').at(-1)]??0.03]));
  }};
}
