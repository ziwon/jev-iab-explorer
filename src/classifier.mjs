...
import {purposeQuestions,assessPurposes} from './purposes.mjs';
import {PURPOSE_ACCEPT,PURPOSE_RUBRIC_VERSION} from '../public/purposes.mjs';
...
export async function classify(input,taxonomy,provider,{demo=false,policy=POLICY}={}) {
  ...
  for(const segment of all.slice(0,policy.maxSegments)) {
    const result={segment_id:segment.id,start_sec:segment.start,end_sec:segment.end,status:'insufficient_evidence',sufficiency_probability:null,abstention_reason:null,labels:[],evaluations:[],warnings:[],evidence:segment.evidence,purpose_assessment:{status:'not_assessed',reason:'not_evaluated',labels:[],evaluations:[]}};
    output.push(result);
    ...
    let halted=false;
    try {
      const enough=await provider.evaluate(state,{...purposeQuestions(evidenceMode),sufficient:{type:'noul',instructions:evidenceMode==='metadata_only'
      ...
      result.purpose_assessment=assessPurposes(enough,enough.sufficient,segment.evidence);
